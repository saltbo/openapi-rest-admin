...
export class ResponseTransformer {
   ...
   */
  static transformListResponseData<T>(
    responseData: any,
    ...
    if (!resourceSchema) {
      throw new APIError('resourceSchema is required for list data transformation', 400, 'Configuration Error');
    }

    // 如果数据为空或null
    if (responseData === null || responseData === undefined) {
      throw new APIError('Expected list response but got null/undefined', 500, 'Parse Error');
    }
    ...
    if (Array.isArray(responseData)) {
      ...
      if (responseData.length > 0) {
        const sampleItem = responseData.find(item => item !== null && item !== undefined);
        if (!sampleItem || !this.isResourceData(sampleItem, resourceSchema)) {
          throw new APIError('Array elements do not match the provided resourceSchema', 500, 'Parse Error');
        }
      }
      return {
    ...
    if (typeof responseData !== 'object') {
      throw new APIError('Expected list response to be object or array', 500, 'Parse Error');
    }

    // 响应是包装对象，基于 schema 查找数组字段
    // 常见的数组数据字段名
    const commonArrayFields = ['data', 'items', 'list', 'results', 'records', 'content'];
    ...
    for (const fieldName of commonArrayFields) {
      if (fieldName in responseData && Array.isArray(responseData[fieldName])) {
        ...
        if (arrayData.length > 0) {
          const sampleItem = arrayData.find(item => item !== null && item !== undefined);
          if (!sampleItem || !this.isResourceData(sampleItem, resourceSchema)) {
    ...
    for (const [fieldName, fieldValue] of Object.entries(responseData)) {
      if (Array.isArray(fieldValue)) {
        ...
        if (fieldValue.length > 0) {
          const sampleItem = fieldValue.find(item => item !== null && item !== undefined);
          if (!sampleItem || !this.isResourceData(sampleItem, resourceSchema)) {
            continue; // 不符合 schema，继续查找其他字段
          }
        }
    ...
    throw new APIError('Could not extract list data using provided resourceSchema', 500, 'Parse Error');
  }
   ...
   * 要求调用方必须传入 resourceSchema，只用 resourceSchema 匹配 response
   */
  static transformSingleResponseData<T>(
    responseData: any,
    resourceSchema: OpenAPIV3.SchemaObject
  ): T {
    if (!resourceSchema) {
      throw new APIError('resourceSchema is required for single resource transformation', 400, 'Configuration Error');
    }
    ...
    // 情况 2: 响应是包装对象，查找资源字段
    // 常见的单个资源数据字段名
    const commonResourceFields = ['data', 'item', 'result', 'record', 'content'];
    ...
    throw new APIError('Could not extract single resource data using provided resourceSchema', 500, 'Parse Error');
  }

  /**
   * 创建默认分页信息
   */
  private static createDefaultPagination(totalItems: number): PaginationInfo {
    return {
      page: 1,
      pageSize: totalItems,
      total: totalItems,
   ...
   * 只检查第一层字段结构，不做深度递归验证
   */
  private static isResourceData(data: any, resourceSchema: OpenAPIV3.SchemaObject): boolean {
   ...
   * 只保留基本的分页信息提取逻辑
   */
  private static extractPaginationFromObject(obj: any): PaginationInfo {
    let page = 1;
    let pageSize = 20;
    let total = 0;
